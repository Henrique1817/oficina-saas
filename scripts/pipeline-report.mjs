#!/usr/bin/env node
/**
 * Reporta run/step da pipeline (opcional).
 *
 * Uso:
 ...
 *
 * Env: PIPELINE_INGEST_URL, PIPELINE_INGEST_SECRET (+ GITHUB_*).
...
if (cmd === "run") {
  const body = {
    ...
    workflow: env(
      "GITHUB_WORKFLOW",
      typeof flags.workflow === "string" ? flags.workflow : "unknown",
    ),
...
if (cmd === "step") {
  ...
  console.log("[pipeline-report] step ok", data?.id || flags.name);
  process.exit(0);
}
