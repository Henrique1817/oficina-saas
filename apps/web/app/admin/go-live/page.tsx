...
type Check = { id: string; label: string; ok: boolean; href?: string };
...
export default async function GoLivePage() {
  ...
  const checks: Check[] = [
    {
      id: "access",
      label: "Plano com acesso (ACTIVE ou trial válido)",
    ...
    {
      id: "stripe-customer",
      label: "Customer Stripe vinculado à oficina",
      ok: Boolean(org.stripeCustomerId),
      href: "/billing",
    },
    {
      id: "customer",
  ...
  const required = checks.filter((c) => c.id !== "team");
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Critérios “bom o bastante” para cobrar de verdade · {org.name}
        </p>
      </div>
      ...
      <p className={`text-sm font-medium ${ready ? "text-success" : "text-accent"}`}>
        {ready
          ...
          : "Ainda faltam itens obrigatórios (tudo exceto o 2º usuário)."}
      </p>

      <p className="text-sm text-muted-foreground">
        Playbook completo: <code className="text-xs">ops/fase-4-soft-launch.md</code>
      </p>
    </div>
  );
}
